				       ...
				       GError **error)
{
	for (guint i = 0; i < bufsz; i++) {
		...
		if (!fu_bcm57xx_recovery_device_bar_write (self, FU_BCM57XX_BAR_DEVICE,
			...
			return FALSE;
		tmp.bits.Doit = 1;
		/* write one word at a time to avoid data corruption issue */
		tmp.bits.First = TRUE;
		tmp.bits.Last = TRUE;
		if (!fu_bcm57xx_recovery_device_bar_write (self, FU_BCM57XX_BAR_DEVICE,
							   REG_NVM_COMMAND, tmp.r32, error))
					     ...
					     GError **error)
{
	...
	fw_old = fu_bcm57xx_recovery_device_dump_firmware (device, error);
	if (fw_old == NULL)
		return NULL;
...
fu_bcm57xx_recovery_device_setup (FuDevice *device, GError **error)
{
	...
	if (fwversion != 0x0) {
		...
		/* this is only set on the non-free firmware */
...
fu_bcm57xx_recovery_device_new (GUdevDevice *udev_device)
{
	FuUdevDevice *self = g_object_new (FU_TYPE_BCM57XX_RECOVERY_DEVICE,
